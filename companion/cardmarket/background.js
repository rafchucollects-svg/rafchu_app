...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  ...
  (async () => {
    try {
      ...
      let data;
      if (message.action === 'status') data = { installed: true, version: chrome.runtime.getManifest().version, runId: stored.report?.runId, productRunId: stored.products?.runId, productRevision: stored.products ? `${stored.products.runId}:${stored.products.revision ?? stored.products.results.length}` : null, reportRevision: stored.report ? `${stored.report.runId}:${stored.report.revision || 0}:${stored.report.captures.length}:${stored.report.errors?.length || 0}` : null, canResume: Boolean(stored.captureJob && !captureRunner.active), hasCaptureJob: Boolean(stored.captureJob), hasSuggestionJob: Boolean(stored.suggestionJob), canResumeSuggestions: Boolean(stored.suggestionJob && !suggestionRunner.active), capabilities: ['product-suggestions', 'resumable-capture', 'seller-photo-cache', 'promo-product-lookup', 'resilient-product-search', 'resumable-product-search', 'server-error-recovery', 'combined-product-offers', 'bounded-discovery-storage', 'capture-pagination-recovery'], status: !suggestionRunner.active && !captureRunner.active && stored.status?.state === 'running' ? { state: stored.captureJob || stored.suggestionJob ? 'paused' : 'error', message: stored.captureJob ? 'Capture interrupted. Resume to continue from the last completed card.' : stored.suggestionJob ? 'Product search interrupted. Resume to continue from the unfinished card.' : 'Search interrupted. Retry suggestions.' } : stored.status };
        ...
        data = stored.products ? { ...stored.products, photos: fresh ? cache.images : {}, ...(fresh && cache.warning ? { photoWarning: cache.warning } : {}) } : null;
      }
        ...
        data = stored.report ? { ...stored.report, photos: fresh ? cache.images : {}, ...(fresh && cache.warning ? { photoWarning: cache.warning } : {}) } : null;
      }
      ...
      else if (message.action === 'start' || message.action === 'retry-failed') {
        ...
        if (message.action === 'retry-failed') {
          const now = Date.now();
          const bindingFields = ['inventoryKey', 'productUrl', 'language', 'condition', 'finish', 'firstEdition', 'confirmed'];
          if (!stored.report?.runId || !Array.isArray(stored.report.captures) || !Array.isArray(stored.report.errors) ||
            new Set(message.tasks.map(task => task.entryId)).size !== message.tasks.length ||
            message.tasks.some(task => task.binding.confirmed !== true || !stored.report.errors.some(failure => failure?.entryId === task.entryId &&
              failure.inventoryKey === task.binding.inventoryKey && failure.binding &&
              bindingFields.every(key => failure.binding[key] === task.binding[key]) &&
              Date.parse(failure.failedAt) <= now + 300000 && now - Date.parse(failure.failedAt) < 86400000))) {
            throw new Error('Retry only cards that failed with these confirmed product filters. Refresh listings if a match has changed.');
          }
          void captureRunner.run(message.tasks, null, stored.report);
        } else void captureRunner.run(message.tasks);
        data = { started: true };
