...
export function CardmarketSyncPanel({ onClose }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3"><section aria-label="Cardmarket variant sync" className="max-h-[95vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-xl">
    ...
    {status?.installed && !status?.capabilities?.includes('resilient-product-search') && <p className="mt-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-900">Improved SM promo, EX/GX and Gold Star lookup is available in companion 0.3.3. Update the companion and refresh Rafchu, then retry Suggest product links.</p>}
