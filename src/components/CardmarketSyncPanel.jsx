...
export function CardmarketMatchForm({ item, busy, candidates = [], previews = [], previewErrors = [], photos = {}, lookupError, lookupCode, displayPreferences = { currency: 'EUR' }, onSave }) {
  ...
  return <details className="mt-3 rounded-lg border border-slate-200 p-3" open={!saved}>
    ...
    {preview && <details className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3" aria-label={`Captured listing preview for ${item.entryId}`}>
      ...
      <p className="mt-2 text-xs">These listings belong to the product above. Confirm its language, condition and printing below to choose a matching price without another capture.</p>
      {(!preview.complete || preview.error) && <p className="mt-2 text-xs text-amber-800">{preview.error || 'The listing preview is incomplete. Refresh this product before applying a price.'}</p>}
      <div className="mt-2 max-h-64 space-y-2 overflow-y-auto">{preview.offers.map(offer => <div key={offer.offerId} className="flex gap-3 rounded border border-blue-100 bg-white p-2 text-xs">
        <span className="min-w-0 flex-1"><strong>{money(offer.price)}</strong> · {offer.seller}<span className="block">{offer.condition} · {offer.language || 'Unknown language'} · {offer.finish === 'reverse' ? 'Reverse Holo' : 'Non-reverse'} · {offer.firstEdition ? '1st Edition' : 'Not first edition'}</span><a className="text-blue-800 underline" href={offer.url} target="_blank" rel="noopener noreferrer">View listing</a></span>
        {offer.scanUrl && <CardmarketPhoto src={offer.scanUrl} cachedSrc={photos[offer.scanUrl]} name={`${item.name} offered by ${offer.seller}`} productUrl={offer.url} seller />}
      </div>)}</div>
    </details>}
...
export function CardmarketSyncPanel({ onClose }) {
  const { user, db, collectionItems = [], currency = 'EUR', secondaryCurrency, roundUpPrices = false } = useApp();
  ...
  const productCaptureRun = useRef(null);
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (productRevision && productRun.current !== productRevision) {
          ...
          if (alive) {
            setProductResults(result?.results || []); setProductReport(result);
            if (productCaptureRun.current && productCaptureRun.current !== result?.runId) { setSelected({}); setExcluded({}); setPriceFeedback(null); }
            productCaptureRun.current = result?.runId; productRun.current = productRevision;
          }
        }
  ...
  // Status polling also rechecks expiry while the review stays open.
  const reviewNow = Date.now();
  const reviewReport = cardmarketReviewReport(items, report, productReport, reviewNow);
  const summaries = Object.fromEntries(items.map(item => { try { return [item.entryId, summarizeCardmarketOffers(item, reviewReport?.captures?.find(row => row.entryId === item.entryId), reviewNow)]; } catch (err) { return [item.entryId, { status: 'error', reason: err.message, offers: [] }]; } }));
  const captureErrors = Object.fromEntries(linked.flatMap(item => {
    if (!report?.runId || report.runId !== status?.runId || summaries[item.entryId].status === 'needs-match') return [];
    const failure = (Array.isArray(report.errors) ? report.errors : []).find(row => row?.entryId === item.entryId &&
      row.inventoryKey === cardmarketInventoryKey(item) && typeof row.error === 'string' && row.error.trim() &&
      row.binding && ['inventoryKey', 'productUrl', 'language', 'condition', 'finish', 'firstEdition', 'confirmed'].every(key => row.binding[key] === item.cardmarketBinding[key]) &&
      Date.parse(row.failedAt) <= reviewNow + 300000 && reviewNow - Date.parse(row.failedAt) < 86400000);
    return failure ? [[item.entryId, failure]] : [];
  }));
  ...
  const saveChosenPrices = async () => {
    ...
    try {
      const result = await saveCardmarketOffers(db, user.uid, reviewReport, choices);
  ...
  const combinedCapture = status?.capabilities?.includes('combined-product-offers');
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3"><section aria-label="Cardmarket variant sync" className="max-h-[95vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-xl">
    <div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-semibold">Cardmarket · ungraded singles</h2><p className="mt-1 text-sm text-slate-600">Find product links and listings together. Review the match, then choose a price from the captured offers. Asking prices per card, excluding shipping. Displayed in {currency}{secondaryCurrency && secondaryCurrency !== currency ? ` (${secondaryCurrency})` : ''}.</p></div><Button variant="ghost" onClick={onClose}>Close</Button></div>
    ...
    {unlinked.length > 0 && <p className="mt-1 text-xs text-slate-600">The initial lookup also collects listings for the suggested products. Confirm each match below to use those listings. Ordinary cards and promos are included without variant tags.</p>}
    ...
    {status?.installed && !combinedCapture && <p className="mt-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-900">Companion 0.3.6 collects links and listings together. Update the companion to use prices from the initial lookup without another capture.</p>}
    {status?.installed && combinedCapture && !status?.capabilities?.includes('bounded-discovery-storage') && <p className="mt-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-900">Companion 0.3.7 fixes pauses while Cardmarket loads the next page and makes more room for captured listings. Download the update below, reload it in Chrome, and refresh Rafchu and the reader. Your saved capture can then resume.</p>}
    {status?.installed && !status?.capabilities?.includes('capture-pagination-recovery') && <p className="mt-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-900">Companion 0.3.8 retries a stalled offer page and continues with the remaining cards if that product still cannot load. Download the update below, reload it in Chrome, then refresh Rafchu and resume your saved capture.</p>}
    {[...new Set([report?.photoWarning, productReport?.photoWarning].filter(Boolean))].map(warning => <p className="mt-2 text-xs text-slate-600" key={warning}>{warning}</p>)}
    <div className="mt-3 flex flex-wrap gap-2"><Button variant="outline" disabled={busy || !status?.installed || !status?.capabilities?.includes('product-suggestions') || !unlinked.length || capturePending || searchPending || status?.status?.state === 'running'} onClick={() => action(async () => { await cardmarketRequest('suggest', unlinked.map(item => ({ entryId: item.entryId, name: item.name, set: cardmarketTarget(item).set, number: item.number, language: cardmarketTarget(item).language, inventoryKey: cardmarketInventoryKey(item), ...(combinedCapture ? { captureOffers: true, condition: cardmarketTarget(item).condition } : {}) }))); setMessage(combinedCapture ? 'Finding links and collecting listings together. Review each match before using its prices.' : 'Finding product suggestions for unmatched cards. Your edited URLs and saved matches will be kept.'); })}>{combinedCapture ? 'Find links and listings' : 'Suggest product links'}</Button><Button disabled={busy || !status?.installed || !linked.length || capturePending || searchPending || status?.status?.state === 'running'} onClick={() => action(async () => { await cardmarketRequest('start', linked.map(item => ({ entryId: item.entryId, name: item.name, binding: item.cardmarketBinding }))); setMessage('Capture started. Leave the Cardmarket reader tab open.'); })}>{combinedCapture ? `Refresh listings for ${linked.length} matched ${linked.length === 1 ? 'card' : 'cards'}` : `Capture ${linked.length} linked ${linked.length === 1 ? 'card' : 'cards'}`}</Button>
      ...
      {failedLinked.length > 0 && !capturePending && !searchPending && status?.status?.state !== 'running' && <Button variant="outline" disabled={busy || !status?.installed || !status?.capabilities?.includes('capture-pagination-recovery')} onClick={() => action(async () => { await cardmarketRequest('retry-failed', failedLinked.map(item => ({ entryId: item.entryId, name: item.name, binding: item.cardmarketBinding }))); setMessage(`Retrying listings for ${failedLinked.length} failed ${failedLinked.length === 1 ? 'card' : 'cards'}. Completed offers are kept and your current prices are unchanged.`); })}>Retry {failedLinked.length} failed matched {failedLinked.length === 1 ? 'card' : 'cards'}</Button>}
      ...
      return <article className="rounded-xl border border-slate-200 p-4" key={id}>
        ...
        {reviewReport && <div className="mt-3">{(!captureError || capture) && <p className="text-sm">{!capture && capturePending ? 'Waiting for this card. Completed offers will appear here automatically.' : !capture ? 'No offers captured for this card yet.' : summary.status === 'ready' ? `${summary.offers.length} matching ${summary.offers.length === 1 ? 'offer' : 'offers'} from ${summary.sellerCount} ${summary.sellerCount === 1 ? 'seller' : 'sellers'}` : summary.reason || 'No matching offers. Current price stays unchanged.'}</p>}
          ...
          {summary.offers.length > 0 && <><p className="mt-1 text-xs text-slate-500">{summary.excluded} offers excluded for condition, variant, language, signatures, alterations or grading. Choose one matching offer below.</p><p className="mt-1 text-xs text-slate-500">Captured {new Date(capture.capturedAt).toLocaleString()}. Product references may show a different printing; seller photos show the advertised copy.</p>{professionalOnly && !summary.offers.some(offer => ['Professional', 'Powerseller'].includes(offer.sellerType)) && <p className="mt-2 text-sm text-slate-600">No matching Professional or Powerseller offers. Turn off the seller filter to see private listings.</p>}<div className="mt-2 max-h-96 space-y-2 overflow-y-auto">{summary.offers.filter(offer => !professionalOnly || ['Professional', 'Powerseller'].includes(offer.sellerType)).map(offer => <label className={`flex flex-wrap items-start gap-3 rounded-lg border p-3 text-sm ${selected[id] === offer.offerId ? 'border-blue-400 bg-blue-50' : 'border-slate-200'}`} key={offer.offerId}>
            ...
            <span className="min-w-40 flex-1"><span className="font-semibold">{money(offer.price)}</span> · {offer.seller} · {offer.country}<span className={`ml-2 inline-block rounded px-1.5 py-0.5 text-xs font-semibold ${['Professional', 'Powerseller'].includes(offer.sellerType) ? 'bg-emerald-100 text-emerald-900' : 'bg-slate-100 text-slate-700'}`}>{offer.sellerType || 'Seller type unavailable'}</span><span className="mt-1 block text-xs text-slate-600">{offer.condition} · {offer.language} · {offer.finish === 'reverse' ? 'Reverse Holo' : 'Non-reverse'} · {offer.firstEdition ? '1st Edition' : 'Not first edition'}</span>{offer.comments && <span className="mt-1 block text-xs">{offer.comments}</span>}<a href={offer.url} target="_blank" rel="noopener noreferrer" className="mr-3 text-xs text-blue-800 underline">View offer</a></span><CardmarketPhoto key={offer.scanUrl} src={offer.scanUrl} cachedSrc={reviewReport?.photos?.[offer.scanUrl]} name={`${item.name} offered by ${offer.seller}`} productUrl={offer.url} seller />
    ...
    {reviewReport && <section aria-label="Save chosen Cardmarket prices" aria-busy={savingPrices} className="sticky bottom-0 mt-4 border-t bg-white py-3">
