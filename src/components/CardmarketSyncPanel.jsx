...
export function CardmarketMatchForm({ item, busy, candidates = [], previews = [], previewErrors = [], photos = {}, lookupError, lookupCode, onSave }) {
  ...
  return <details className="mt-3 rounded-lg border border-slate-200 p-3" open={!saved}>
    ...
    {preview && <details className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3" aria-label={`Captured listing preview for ${item.entryId}`}>
      ...
      <div className="mt-2 max-h-64 space-y-2 overflow-y-auto">{preview.offers.map(offer => <div key={offer.offerId} className="flex gap-3 rounded border border-blue-100 bg-white p-2 text-xs">
        <span className="min-w-0 flex-1"><strong>{eur(offer.price)}</strong> · {offer.seller}<span className="block">{offer.condition} · {offer.language || 'Unknown language'} · {offer.finish === 'reverse' ? 'Reverse Holo' : 'Non-reverse'} · {offer.firstEdition ? '1st Edition' : 'Not first edition'}</span><a className="text-blue-800 underline" href={offer.url} target="_blank" rel="noopener noreferrer">View listing</a></span>
